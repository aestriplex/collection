...
template <typename T>
flist<flist<T> > flist<T>::group_by(std::function<bool(T,T)> f)
{
...
{
    return this->group_by([](int x,int y) { return x == y; });
}
...
template <typename T>
inline bool flist<T>::map_contains(std::map<T,bool> m, T val)
{
